...
APLICAMOS LA SHELL ZSH POR DEFECTO (es opcional, si queres seguir en bash , no es obligatorio..es tema de gustos,etc) ; 
...
cd~/.config/             (Es un directorio oculto en tu /HOME, ahy se encuentra casi todos los .config del sistema)
cd ~/.config/bspwm/  (Gestor ventanas, es el cerebro de toda la interfaz)
cd ~/.config/sxhkd/   (Gestor de Shortcuts + )
cd~/.config/polybar   (Barra de estados Polybar)
cd~/.config/picom     (Gestor de transparencias y efectos,blurring sobre ventanas etc)
cd~/.config/feh         (Sencillo Gestor de fondos de Pantalla)
